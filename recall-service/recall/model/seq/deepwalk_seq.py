...
def build_seq(rating_df: DataFrame, spark: SparkSession):
    ...
    if os.path.isfile('output/entrance_items.dill'):
        ...
        print('watch_seq is ' + str(watch_seq[0]))
    ...
    for i in range(n):
        ...
        s = one_random_walk(length, entrance_items, entrance_probs, transfer_probs)    ## build 10 neighbors
        samples.append(s)
    print(f'Random walk generated {n} samples.')

    return spark.createDataFrame([[row] for row in samples], ['anime_ids'])

## animeId1, animeId2 => matrix[animeId1][animeId2], matrix[animeId2][animeId1]
...
## probability of neighbor animeId for each anime
